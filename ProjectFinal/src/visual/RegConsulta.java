...
public class RegConsulta extends JDialog {
	...
	private void construirPanelIzquierdoDatos(JPanel panelPadre) {
		...
		if (sangreActual != null)
			cbTipoSangre.setSelectedItem(sangreActual);
		...
		txtPeso = new JTextField(
				String.valueOf(citaActual.getPaciente().getPeso() != null ? citaActual.getPaciente().getPeso() : 0.0f));
		...
		txtEstatura = new JTextField(String.valueOf(
				citaActual.getPaciente().getEstatura() != null ? citaActual.getPaciente().getEstatura() : 0.0f));
		...
		calcularIMC();
		KeyAdapter calcularListener = new KeyAdapter() {
			@Override
			public void keyReleased(KeyEvent e) {
				calcularIMC();
			}
		};
		txtPeso.addKeyListener(calcularListener);
		txtEstatura.addKeyListener(calcularListener);
		...
		panelDesplazamientoSintomas.setBounds(20, 340, 360, 230); // Ajustada altura
	...
	private void construirPanelDerechoAcciones(JPanel panelPadre) {
		...
		TitledBorder bordeTitulo = BorderFactory.createTitledBorder(BorderFactory.createLineBorder(colorAzulAccion),
				"Gestión de Inmunización");
	...
	private void registrarNuevaEnfermedad() {
		...
		Object[] mensaje = { "Nombre de la Enfermedad:", textoNombre, "Vigilancia:", checkVigilancia };

		int opcion = JOptionPane.showConfirmDialog(this, mensaje, "Registrar Nueva Enfermedad",
			...
			if (!nombre.isEmpty()) {
				if (Clinica.getInstance().buscarEnfermedadPorNombre(nombre) == null) {
					Enfermedad nuevaEnfermedad = new Enfermedad(
							Clinica.getInstance().getCatalogoEnfermedades().size() + 1, nombre,
							checkVigilancia.isSelected());
					...
					JOptionPane.showMessageDialog(this, "Esta enfermedad ya existe.", "Error",
	...
	private void solicitarStockVacuna() {
		String nombreVacuna = (String) comboBoxVacunas.getSelectedItem();
		JOptionPane.showMessageDialog(this, "Solicitud de pedido enviada a Administración para: " + nombreVacuna,
	...
	private void agregarDiagnosticoALista() {
		...
		if (tratamiento.isEmpty()) {
			JOptionPane.showMessageDialog(this, "Ingrese el tratamiento o receta.", "Aviso",
		...
		for (Diagnostico diag : listaDiagnosticosTemporal) {
			if (diag.getNombre().equals(nombreEnfermedad)) {
				JOptionPane.showMessageDialog(this, "Esta enfermedad ya fue agregada.", "Aviso",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
		}
	...
	private void finalizarConsultaMedica() {
		...
		if (sintomasPaciente.isEmpty()) {
			JOptionPane.showMessageDialog(this, "Debe registrar los síntomas del paciente.", "Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		if (listaDiagnosticosTemporal.isEmpty()) {
			int confirmacion = JOptionPane.showConfirmDialog(this,
					"No ha agregado ningún diagnóstico.\n¿Desea finalizar solo con los síntomas?", "Confirmar",
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Verifique que el Peso y Estatura sean numéricos.",
					"Error en Datos Físicos", JOptionPane.WARNING_MESSAGE);
			return;
		}
		...
		JOptionPane.showMessageDialog(this, "Consulta finalizada exitosamente.", "Éxito",
