...
public class ListarCitas extends JDialog {
    ...
    private void guardarCambios() {
        ...
        if (citaSeleccionada.getEstado().equalsIgnoreCase("Realizada") || citaSeleccionada.getEstado().equalsIgnoreCase("Cancelada")) {
             JOptionPane.showMessageDialog(this, "No se puede modificar una cita ya finalizada o cancelada.", "Error", JOptionPane.ERROR_MESSAGE);
             return;
        }

        if (nuevoEstado.equalsIgnoreCase("Realizada")) {
            JOptionPane.showMessageDialog(this, "El estado 'Realizada' solo se asigna automáticamente al finalizar la consulta médica.\nUtilice el botón 'Atender'.", "Acción Restringida", JOptionPane.WARNING_MESSAGE);
            cbEstado.setSelectedItem(citaSeleccionada.getEstado());
            return;
        }
    ...
    private void abrirConsulta() {
        if(citaSeleccionada != null) {
            if (citaSeleccionada.getDoctor().getCupoDia() <= 0) {
                JOptionPane.showMessageDialog(this, "El doctor " + citaSeleccionada.getDoctor().getNombre() + " no tiene cupo disponible por hoy.\n(Cupo Actual: " + citaSeleccionada.getDoctor().getCupoDia() + ")", "Sin Cupo", JOptionPane.ERROR_MESSAGE);
                return;
            }
