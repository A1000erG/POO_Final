...
public class ListarDoctores extends JDialog {
	...
	private void loadDoctores(String filtro) {
		...
		for (Doctor doc : lista) {
			...
			if (filtro.isEmpty() || nombre.contains(filtroMin) || idRaw.contains(filtroMin)
					|| idFormatted.contains(filtroMin) || idFull.contains(filtroMin)) {
	...
	private void cambiarFotoDoctor() {
		...
		if (resultado == JFileChooser.APPROVE_OPTION) {
			...
			Image imgEscalada = iconOriginal.getImage().getScaledInstance(lblFoto.getWidth(), lblFoto.getHeight(),
					Image.SCALE_SMOOTH);
	...
	private void modificarDoctor() {
		...
		if (txtNombre.getText().isEmpty() || txtEspecialidad.getText().isEmpty() || txtCupo.getText().isEmpty()
				|| pass.isEmpty()) {
		...
		try {
			...
			if (nuevaRutaFotoTemp != null) {
				try {
					...
					JOptionPane.showMessageDialog(this, "Error al actualizar la foto: " + e.getMessage(), "Error IO",
							JOptionPane.ERROR_MESSAGE);
				}
			}

			Clinica.getInstance().guardarDatos();
			JOptionPane.showMessageDialog(this, "Doctor modificado correctamente.", "Éxito",
					JOptionPane.INFORMATION_MESSAGE);
			loadDoctores(txtBuscador.getText());

		} catch (NumberFormatException e) {
			JOptionPane.showMessageDialog(this, "El cupo debe ser un número válido.", "Error de Formato",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void cambiarEstadoDoctor() {
		...
		if (usuarioActual instanceof Doctor) {
			JOptionPane.showMessageDialog(this, "Acceso Denegado: Solo un Administrador puede inhabilitar Doctores.",
					"Permisos Insuficientes", JOptionPane.ERROR_MESSAGE);
			return;
		}

		if (selectedDoctor.isActivo()) {
			String razon = JOptionPane.showInputDialog(this, "Ingrese la razón de la inhabilitación:",
					"Confirmar Inhabilitación", JOptionPane.WARNING_MESSAGE);
