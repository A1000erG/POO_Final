...
public class ServidorRespaldo extends Thread {
    ...
    /*
    Función: ServidorRespaldo (Constructor)
    Argumentos: (int) puerto: El puerto TCP donde escuchar (ej. 7001).
    Objetivo: Configurar el servidor de respaldo.
    Retorno: Ninguno.
    */
    ...
    /*
    Función: run (De Thread)
    Argumentos: Ninguno.
    Objetivo: Bucle principal que acepta conexiones entrantes de la clínica.
    Retorno: (void).
    */
    @Override
    public void run() {
    ...
    /*
    Función: recibirDatos
    Argumentos: (Socket) socketCliente: La conexión con la aplicación Cliente.
    Objetivo: Leer el archivo binario y el reporte de texto enviados por la red y guardarlos en disco.
    Retorno: (void).
    */
    private void recibirDatos(Socket socketCliente) {
        ...
        try {
            ...
            int leidos;
            long totalLeido = 0;
            while (totalLeido < tamanoArchivo && (leidos = dataInput.read(buffer, 0, (int)Math.min(buffer.length, tamanoArchivo - totalLeido))) != -1) {
            ...
            try {
                if (fileOutBin != null) fileOutBin.close();
                if (fileOutTxt != null) fileOutTxt.close();
